...
class manage_blog  {
	...
	*/
	function _init() {
		...
		$this->_blog_authors = db()->get_2d("SELECT `id`,`name` FROM `".db('blog_authors')."`");
	}
	...
	*/
	function _show_quick_filter () {
		$a = [];
		$a[] = a('/@object/filter_save/clear/?filter=active:0', 'Filter inactive articles', 'fa fa-ban', '', 'btn-warning', '');
		$a[] = a('/@object/filter_save/clear/?filter=active:1', 'Filter active articles', 'fa fa-check', '', 'btn-success', '');
		...
		return
			...
			table(from(self::table), [
				...
				'filter_params' => [
					...
					'cat_id'		=> function($a){ return $a['value'] ? '`id` IN(SELECT object_id FROM '.db('object_cats').' WHERE `object` = "blog" AND cat_id = '.(int)$a['value'].')' : ''; },
			...
			->text('id')
   			->text('title')
			->func('author_id', function($in, $e, $a) {
				return $this->_blog_authors[$in];
			})
			->func(t('categories'), function($in, $e, $a) {
				$c = _class('cats');
				return $c->_get_categories_str_for_object($a['id'], $c->blog_category, 'blog');
			})
			...
			->footer_link('Blog categories', url('/category_editor/drag_items/1'))
			->footer_link('Blog authors', url('/manage_blog_authors'))
		;
	}
	...
	function add() {
		...
		->on_validate_ok(function() {
			db()->insert_safe(self::table, [
				'title'			=> $_POST['title'],
				'lang'			=> $_POST['lang'],
				'url'			=> module('blog')->_name_to_url($title),
				'add_date'		=> time(),
				'publish_date'	=> time(),
				'active'		=> 0,
			...
			common()->message_info('Article was added');
			return js_redirect('/@object/edit/'. $article_id);
		})
		->text('title', ['desc' => 'Article title'])
		->save();
	}
	...
	*/
	function edit() {
		return _class('manage_blog_edit', 'admin_modules/manage_blog/')->{__FUNCTION__}();
	}
	...
	*/
	function delete() {
		...
		$blog_info = module('blog')->_get_info($blog_id, false, $no_cache = true);
		if (!empty($blog_info['id'])) {
			db()->delete(self::table, $blog_id);
		}
		if ($_POST['ajax_mode']) {
			main()->NO_GRAPHICS = true;
			echo $blog_id;
		} else {
			common()->message_info('Article was deleted');
	...
	*/
	function activate() {
		...
		if (!empty($blog_info['id'])) {
			...
			db()->update_safe(self::table, $values, $blog_id);
		}
		if ($_POST['ajax_mode']) {
			main()->NO_GRAPHICS = true;
			echo $active;
	...
	*/
	function _show_filter() {
		...
		$c = _class('cats');
		$cats = $c->_prepare_for_box($c->blog_category);
		unset($cats[' ']);
		...
		return form($r, ['filter' => true])
			...
			->select_box('cat_id', $cats, ['desc' => t('Categories'), 'no_label' => 1, 'show_text' => '-- '.t('Category').' --'])
			->select_box('author_id', $this->_blog_authors, ['desc' => t('Authors'), 'no_label' => 1, 'show_text' => '-- '.t('Author').' --'])
